...
| **The stylist has no key — and, as of #496, does not need one for the shaped asks.** `ANTHROPIC_API_KEY` is in no `.env`, no `config.toml`, no repo secret. With #496, `plan.ts` answers a routine, the gaps, try-next, compare and about from data with zero model calls; only a free-form question reaches `model.ts`, and without a key it gets the honest menu instead of a 503. `STYLIST_DEMO=1` still drives the canned stylist | Sean (key) |
