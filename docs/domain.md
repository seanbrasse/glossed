...
**What counts as disclosure.** A claim that matches an *identifiable person* to a cohort discloses that person's membership in it, **whatever the wording**. If the viewer knows which cohort they are in, "also has your skin type" tells them the other person's skin type exactly as plainly as naming it — the match is the disclosure, not the string. So omitting the value is not a mitigation: any surface pairing a named person with a cohort match needs the same consent as one that prints the value outright. (Worked example: `tech/02` §3.5, where suggestion reasons are gated on `profile_badges` rather than softened in copy.) The companion rule for claims about a *group* is that a cohort-named claim must say whose n it is — see `tech/01` §3. Both fail the same way when a cohort is implied rather than stated.
