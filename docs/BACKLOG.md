...
- [ ] **`agg_variant_stats` has no writer** — and it is the read side of the search moat, not just the payoff. [GLO-157](https://linear.app/glossed/issue/GLO-157). Migrations 0001–0030 create the table, enable RLS on it, and read it, but nothing populates it. Two consequences. The visible one: `payoff_for_variant()` returns `n=0, evidence_backed=false` for every variant, so onboarding shows its neutral fallback 100% of the time and would keep doing so against a fully seeded catalog. The larger one: `chip_counts` lives in this table, and `domain.md` + `tech/01` §5 both say **chips are queries** — *"foundations that don't oxidize, rated by people with my skin"* is the stated moat. [GLO-154](https://linear.app/glossed/issue/GLO-154) gave that sentence 171 words to ask with and #210 gave it a place to write them, but there is still no aggregate to ask across users. **Not a seed problem** — the seed is fine, there is simply no refresh job; worth saying because from outside it looks exactly like missing seed data. Found while building trending ([GLO-127](https://linear.app/glossed/issue/GLO-127)), which needed velocity this table cannot express and got its own `refresh_trending()` in 0030. A Phase-1 gap, not a 1.5 one, and the reason the thresholds above could not be tuned from data.
- [ ] **Chip aggregate min-n — pick it before the writer ships.** [GLO-157](https://linear.app/glossed/issue/GLO-157). A `chip_counts` value of 1 in a skin-type cohort is one person's opinion wearing the clothes of an aggregate — the failure mode min-n exists to prevent everywhere else here (the leaderboard already refuses below 5 per scope and renders "not enough face-offs yet · k of 5"). Easier to hit than it looks: `agg_variant_stats` is keyed by `(variant_id, tone_band, skin_type)`, so a popular shade fragments across cohorts before anyone counts, and the *product* can be well-attested while every cell is n=1. Choose it against the three thresholds above, not independently.
