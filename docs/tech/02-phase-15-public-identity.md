...
- ⚠️ **Both reason kinds are Regulated data about the CANDIDATE, so both are gated on `profile_badges`** (GLO-122, shipped in 0032). §3.4 states the invariant this section would otherwise break: the badges are *the only path* by which `skin_type`, the anchor variant, and `hair_pattern` reach another human. As originally written, this RPC was a second path with no opt-in, publishing from people whose flags default `false`. The anchor reason now requires `show_anchor`; the skin-type reason requires `show_skin_type`.
  - Phrasing cannot substitute for the gate. "Also has your skin type", with the value omitted, still discloses the candidate's skin type to a viewer who knows their own — **the match is the disclosure, not the string** (`domain.md` §5).
  - Consequence, which is a product fact rather than a defect: badges default false, so **this surface returns nothing until people opt in**. An empty list is the correct behaviour for a population that has not consented to being suggested. The lever for a populated surface is the badge flow, not this gate.
